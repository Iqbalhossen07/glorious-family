...
export default function EditFixedExpensePage() {
  ...
  const [userId, setUserId] = useState('MESS_FUND')
      ...
      try {
        ...
        if (currentSession) {
          const [membersData, expenseData] = await Promise.all([
            ...
            FixedExpenseService.getFixedExpenseById(expenseId)
          ])
          ...
          if (expenseData) {
            ...
            setUserId(expenseData.user_id ? expenseData.user_id : 'MESS_FUND')
            setItemName(expenseData.item_name)
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      Swal.showLoading()
      await FixedExpenseService.updateFixedExpense(expenseId, date, actualUserId, itemName, Number(amount))
      await Swal.fire('Success!', 'Other expense updated successfully.', 'success')
  ...
  return (
    <div style={{ animation: 'fadeIn 0.5s ease', paddingBottom: '5rem', maxWidth: '600px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2rem' }}>
        ...
        <div>
          <h1 style={{ fontSize: '1.75rem', fontWeight: 700, marginBottom: '0.2rem' }}>Edit Other Expense</h1>
          <p style={{ color: 'var(--text-muted)' }}>Update shared miscellaneous expense</p>
        </div>
      </div>
