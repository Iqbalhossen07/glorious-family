...
export default function EditMessInfoPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const data = await MessInfoService.getInfoById(infoId)
        if (data) {
          setTitle(data.title)
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      Swal.showLoading()
      await MessInfoService.updateInfo(infoId, title, description)
      await Swal.fire('Success!', 'Info updated successfully.', 'success')
  ...
  return (
    <div style={{ animation: 'fadeIn 0.5s ease', paddingBottom: '5rem', maxWidth: '600px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2rem' }}>
        ...
        <div>
          <h1 style={{ fontSize: '1.75rem', fontWeight: 700, marginBottom: '0.2rem' }}>Edit Info Card</h1>
          <p style={{ color: 'var(--text-muted)' }}>Update information</p>
        </div>
      </div>
