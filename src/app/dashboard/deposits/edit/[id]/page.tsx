...
export default function EditDepositPage() {
    ...
    const loadData = async () => {
      try {
        ...
        if (currentSession) {
          ...
          if (depositData) {
            setDate(depositData.date || new Date().toISOString().split('T')[0])
            setUserId(depositData.user_id)
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      Swal.showLoading()
      await DepositService.updateDeposit(depositId, date, userId, Number(amount))
      await Swal.fire('Success!', 'Deposit updated successfully.', 'success')
  ...
  return (
    <div style={{ animation: 'fadeIn 0.5s ease', paddingBottom: '5rem', maxWidth: '600px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2rem' }}>
        ...
        <div>
          <h1 style={{ fontSize: '1.75rem', fontWeight: 700, marginBottom: '0.2rem' }}>Edit Deposit</h1>
          <p style={{ color: 'var(--text-muted)' }}>Update deposit details</p>
        </div>
      </div>
